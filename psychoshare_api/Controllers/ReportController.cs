...
public class ReportController : ControllerBase
{
    ...
    [HttpGet("orphaned")]
    public ActionResult<List<ReportResponseDto>> GetOrphanedReports()
    {
        var reports = _daoFactory.DAOReport().GetAll();
        var orphanedReports = new List<Report>();

        foreach (var report in reports)
        {
            var reporterExists = _daoFactory.DAOUser().GetUser(report.ReporterUserId) != null;
            var reportedExists = _daoFactory.DAOUser().GetUser(report.ReportedUserId) != null;

            if (!reporterExists || !reportedExists)
            {
                orphanedReports.Add(report);
            }
        }

        var response = orphanedReports.Select(r => new ReportResponseDto
        {
            Id = r.Id,
            ReporterUserId = r.ReporterUserId,
            ReportedUserId = r.ReportedUserId,
    ...
    [HttpDelete("orphaned")]
    public ActionResult<int> DeleteOrphanedReports()
    {
        var reports = _daoFactory.DAOReport().GetAll();
        var orphanedReports = new List<Report>();
        ...
        return Ok(orphanedReports.Count);
    }
}
